        ...
        success: function (response) {
            ...
            for (let i = 0; i < response.length; i++) {
                let row = `<tr class="bg-white w-full border-b border-gray-600 dark:bg-gray-800 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600">`;
                    ...
                    row +=
                        `<td class="px-6 py-4">
                            <button type="button" onclick="open_modal('/users/request_to_join/${response[i]['institution_id']}/${response[i]['current_user_id']}/', 'drawer_create')" class="px-3 py-2 text-xs font-medium text-center text-white bg-green-700 rounded-lg hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
